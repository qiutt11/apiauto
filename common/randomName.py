...
path = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(__file__))),'data/')
# print(path)
...
class RandomName(object):
    ...
    def __init__(self, csv_path=path):
        ...
        self.first_name_csv = csv.reader(open("{0}first_name.csv".format(csv_path), "r", encoding='utf-8'))
        self.first_name_head = next(self.first_name_csv)
        self.first_name_data = [item for item in self.first_name_csv]
    ...
    def get_name(self, name_len=None):
        '''随机生成女孩名字
        Args：
            name_len: int, 名字长度
        Returns:
        ...
        for i in range(name_length):
            name += random.choices([item[1] for item in self.first_name_data],
                                   weights=[int(item[0]) for item in self.first_name_data])[0]
        return name
...
if __name__ == "__main__":
    ...
    print(random_name_obj.get_name())
